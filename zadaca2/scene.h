...
class Scene 
{
  ...
  public:
    Vec3f cast_ray(const Ray &ray, int bounces) const
    {
        Vec3f hit_point;
        Vec3f hit_normal;
        Object* hit_object;
        ...
        if (scene_intersect(ray, hit_point, hit_normal, hit_object))
        {
            ...
            if (bounces > 0 && (hit_object->material.reflection_coef != 1 || hit_object->material.opacity_coef != 1))
            {
                bounces--;

                Vec3f reflectDir = ray.direction - hit_normal * 2.0f * (ray.direction * hit_normal) ;
                reflectDir.normalize();

                Ray reflection(hit_point + reflectDir * 0.001, reflectDir);

                Vec3f ret = hit_object->material.diffuse_colour * diff * (hit_object->material.reflection_coef) + Vec3f(1, 1, 1) * spec;
                ret = ret + cast_ray(reflection, bounces) * (1 - hit_object->material.reflection_coef);
